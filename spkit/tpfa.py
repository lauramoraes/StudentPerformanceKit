...
class TPFA(PFA):
    ...
    def _transform_data(self, data, q_matrix, n_jobs_transform=None, past_questions=1,
    ...
    def fit(self, data, q_matrix, n_jobs_transform=None, **kwargs):
        ...
        if self.lib == "sklearn":
            ...
            model.fit(X, y)
            self.params["weights"] = np.concatenate((
                model.intercept_.reshape(-1, 1), model.coef_), axis=1)
        elif self.lib == "sm":
